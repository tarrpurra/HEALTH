...
class AudioClient {
  constructor(serverUrl = buildDefaultServerUrl()) {
    ...
    this.userIdSent = false // Track if user_id has been sent
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onclose = (event) => {
          ...
          this.isConnected = false
          this.userIdSent = false // Reset on close
        ...
        this.ws.onmessage = async (event) => {
          try {
            ...
            if (message.type === "ready") {
              ...
              // Send user ID after receiving ready if we have it and haven't sent it
              if (this.userId && !this.userIdSent) {
                this.ws.send(
                  JSON.stringify({
                    type: "user_id",
                    data: this.userId,
